...
from docqa.nn.embedder import WordEmbedder, CharEmbedder, LearnedSubstringEmbedder, SRLTagEmbedder
...
class DocumentAndQuestionEncoder(Configurable):
    ...
    def init(self, input_spec: ParagraphAndQuestionSpec, len_op: bool,
             word_emb: WordEmbedder, tag_emb: Optional[SRLTagEmbedder], char_emb: Optional[CharEmbedder]):

        self._word_embedder = word_emb
        self._char_emb = char_emb
        self._tag_emb = tag_emb
        ...
        self.context_tags = tf.placeholder('int32', [batch_size, n_context_words], name='context_tags')
        ...
        self.question_tags = tf.placeholder('int32', [batch_size, n_question_words], name='question_tags')
        self.question_len = tf.placeholder('int32', [batch_size], name='question_len')

        if self._char_emb:
            self.context_chars = tf.placeholder('int32', [batch_size, n_context_words, self.max_char_dim], name=
                                                'context_chars')
            self.question_chars = tf.placeholder('int32', [batch_size, n_question_words, self.max_char_dim], name=
                                                 'question_chars')
    ...
    def encode(self, batch: List[ContextAndQuestion], is_train: bool):
        ...
        # Setup tag placeholders
        if self._tag_emb is not None:
            context_tags = np.zeros([batch_size, context_word_dim], dtype='int32')
            question_tags = np.zeros([batch_size, ques_word_dim], dtype='int32')
            feed_dict[self.context_tags] = context_tags
            feed_dict[self.question_tags] = question_tags
        else:
            question_tags, context_tags = None, None
        ...
        for doc_ix, doc in enumerate(batch):
            ...
            doc_tag_mapping = {}  # word->ix mapping if `query_once` is True

            for word_tag_ix, word_tag in enumerate(doc.get_question_tag()):
                if self._tag_emb is not None:
                    if query_once:
                        ix = doc_tag_mapping.get(word_tag)
                        if ix is None:
                            ix = self._tag_emb.context_tag_to_ix(word_tag, is_train)
                            doc_tag_mapping[word_tag] = ix
                    else:
                        ix = self._tag_emb.context_tag_to_ix(word_tag, is_train)
                    if word_tag_ix < ques_word_dim:
                        question_tags[doc_ix, word_tag_ix] = ix

            for word_tag_ix, word_tag in enumerate(doc.get_context_tag()):
                if self._tag_emb is not None:
                    if query_once:
                        ix = doc_tag_mapping.get(word_tag)
                        if ix is None:
                        ...
                        ix = self._word_embedder.context_word_to_ix(word_tag, is_train)
                    if word_tag_ix < context_word_dim:
                        context_tags[doc_ix, word_tag_ix] = ix
...
class DocumentAndQuestionEncoderWithSubstring(DocumentAndQuestionEncoder):
    def encode(self, batch: List[ContextAndQuestion], is_train: bool):
        batch_size = len(batch)
        if self.batch_size is not None:
            if self.batch_size < batch_size:
        ...
        if self._char_emb is not None:
            # sanity check
            assert isinstance(self._char_emb, LearnedSubstringEmbedder)
            context_chars = np.zeros([batch_size, context_word_dim, max_char_dim], dtype='int32')
            question_chars = np.zeros([batch_size, ques_word_dim, max_char_dim], dtype='int32')
            context_word_len = np.zeros([batch_size, context_word_dim], dtype='int32')
            question_word_len = np.zeros([batch_size, ques_word_dim], dtype='int32')
        ...
        for doc_ix, doc in enumerate(batch):
            ...
            for word_ix, word in enumerate(doc.question):
                if self._word_embedder is not None:
                    ...
                    question_words[doc_ix, word_ix] = ix
                if self._char_emb is not None:
                    substrings = self._char_emb.get_substrings(word)
                    question_word_len[doc_ix, word_ix] = min(self.max_char_dim, len(substrings))
                    for char_ix, char in enumerate(substrings):
                        if char_ix == self.max_char_dim:
                            break
                        question_chars[doc_ix, word_ix, char_ix] = self._char_emb.char_to_ix(char)
            ...
            for word_ix, word in enumerate(doc.get_context()):
                ...
                if self._char_emb is not None:
                    substrings = self._char_emb.get_substrings(word)
                    context_word_len[doc_ix, word_ix] = min(self.max_char_dim, len(substrings))
                    for char_ix, char in enumerate(substrings):
                        if char_ix == self.max_char_dim:
                            break
                        context_chars[doc_ix, word_ix, char_ix] = self._char_emb.char_to_ix(char)
