...
from docqa.nn.embedder import WordEmbedder, CharWordEmbedder, SRLTagEmbedder
...
SRL_context_train_file = ""
SRL_context_dev_file = ""
SRL_que_train_file = ""
SRL_que_dev_file = ""
...
class ElmoQaModel(Model):
    ...
    def __init__(self,
                 ...
                 tag_embed: Optional[SRLTagEmbedder] = None):
    ...
    def set_input_spec(self, input_spec, voc, word_vec_loader=None):
        ...
        self.encoder.init(input_spec, True, self.word_embed, None if self.tag_embed is None else self.tag_embed,
    ...
    def get_predictions_for(self, input_tensors: Dict[Tensor, Tensor]):
        ...
        # Todo add tag_embed here

        if enc.question_tags in input_tensors:
            with tf.variable_scope("tag-embed"):
                q, c = self.tag_embed.embed(is_train,
                                            (input_tensors[enc.question_tags], q_mask),
                                            (input_tensors[enc.context_tags], c_mask))
...
class AttentionWithElmo(ElmoQaModel):
    ...
    def __init__(self,
                 ...
                 tag_embed: Optional[SRLTagEmbedder],
                 word_embed: Optional[WordEmbedder],
        ...
        :param append_before_atten: Append the language model vectors to the after pre-processing
            the original text
        """
        super().__init__(encoder, lm_model, per_sentence, max_batch_size, word_embed, char_embed, tag_embed)
