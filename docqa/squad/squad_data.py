...
class Question(object):
    ...
    def __init__(self, question_id: str, words: List[str], answer: ParagraphSpans, tag: Optional[List[str]]=None):
...
class Paragraph(ParagraphWithInverse):
    ...
    def __init__(self,
                 ...
                 context_tag: Optional[List[List[str]]] = None):
...
class DocParagraphAndQuestion(ContextAndQuestion):
    ...
    def __init__(self, question: List[str], answer: Optional[Answer],
                 question_id: str, paragraph: Paragraph, question_tag: Optional[List[str]]=None):
    ...
    def get_context_tag(self):
        return flatten_iterable(self.paragraph.context_tag)
...
class SquadCorpus(Configurable):
    ...
    def make_corpus(train: List[Document],
                    dev: List[Document], tag_list: Optional[List[str]]=None):
        dir = join(CORPUS_DIR, SquadCorpus.NAME)
        #if isfile(dir) or (exists(dir) and len(listdir(dir))) >= 2:
        #    raise ValueError("Directory %s already exists and is non-empty" % dir)
        if not exists(dir):
            makedirs(dir)

        for name, data in [(SquadCorpus.TRAIN_FILE, train), (SquadCorpus.DEV_FILE, dev), (SquadCorpus.TAG_LIST_FILE, tag_list)]:
            if data is not None:
