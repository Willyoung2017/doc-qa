...
class MultiSelfAttention(SequenceMapper):
    ...
    def apply(self, is_train, x, mask=None):
        ...
        print("x_feature_dim = ", x_feature_dim, "\n")
        print("batch_size = ", x.shape.as_list()[0], " x_word_dim = ",x.shape.as_list()[1])
        ...
        print("queries", queries.shape.as_list(),"keys",keys.shape.as_list())
        ...
        dist_matrix = tf.einsum("bwhd,bkhd->bwhk", queries, keys)   # dots of (batch, word, key, head)
        ...
        if self.bias:
            ...
            dist_matrix += bias
        select_probs = tf.nn.softmax(dist_matrix)   # for each (batch, word, head) probability over keys # 45 None 5 5
        print(select_probs.shape.as_list())
        memories = tf.tensordot(x, mem_proj, [[2], [0]])  # (batch, memory, head, mem_size)
        print(memories.shape.as_list())
        #memories = tf.reshape(memories, (batch_size, x_word_dim, mem_size, self.n_heads))
        #response = tf.matmul(select_probs, memories, transpose_b=True)
        response = tf.einsum("bwhk,bkhd->bwhd", select_probs, memories)  # (batch, word, head, mem_size)
        print(response.shape.as_list())
        # print("batch_size=",batch_size," x_word_dim=",x_word_dim, "\n")
