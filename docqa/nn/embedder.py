...
from docqa.utils import ResourceLoader, load_pretrained_substring_embedding
...
class LearnedSubstringEmbedder(CharEmbedder):
    """
    learn word embed from sub string embedding.
    """
    def __init__(self, word_size_th: int, char_th: int, char_dim: int,
                 init_scale: float=0.1, force_cpu: bool=False):
        self.force_cpu = force_cpu
    ...
    def _load_substring_vocab(self, sub_vocab_file):
        import os
        from docqa.config import CORPUS_DIR
        self._word_to_substrings = {}

        vocab_path = sub_vocab_file
        if not os.path.exists(sub_vocab_file):
            vocab_path = os.path.join(CORPUS_DIR, "squad", sub_vocab_file)
            if not os.path.exists(vocab_path):
                raise FileNotFoundError("substring vocab file %s not found." % vocab_path)
        with open(vocab_path, 'r', encoding='utf-8') as vocab_file:
            for line in vocab_file.readlines():
                word, subs = line.split('\t')
                subs = subs.split() if subs != '' else list(word)  # fall back to chars if no split choice available
                self._word_to_substrings[word] = subs

    def get_substrings(self, word):
        return self._word_to_substrings.get(word, list(word))
    ...
    def set_vocab(self, corpus):
        ...
        counts = Counter()
        for w,count in w_counts.items():
            for c in self._word_to_substrings.get(w, w):
                counts[c] += count
...
class LearnedSubstringEmbedderWithPretrain(LearnedSubstringEmbedder):

    def init_substring(self, substring_file, pretrained_file, trainable: bool = True):
        self._load_substring_vocab(substring_file)
        self.pretrain_file = pretrained_file
        self.emb_mat = None
        self.trainable = trainable

    def _load_pretrained(self):
        embedding_np = load_pretrained_substring_embedding(self.pretrain_file, self._char_to_ix, self.char_dim)
        saved_embeddings = tf.constant(embedding_np)
        self.emb_mat = tf.Variable(initial_value=saved_embeddings, trainable=self.trainable)
    ...
    def init(self, word_vec_loader, voc: Iterable[str]):
        ...
        print("Loaded pretrain embedding from %s" % self.pretrain_file)
...
class SRLTagEmbedder(Configurable):

    def __init__(self,
                 tag_dim: int,
                 tag_vec_init_scale: float = 0.05,
                 learn_unk: bool = True,
                 keep_probs: float = 1,
                 keep_word: float= 1,
        ...
        self.tag_vec_init_scale = tag_vec_init_scale
        ...
        self._tag_to_vector = None
        self._tag_to_ix = None
        self._tag_emb_mat = None
        self._tag_list = None

    def _load_tags_vector(self):
        tag_list_path = join(config.CORPUS_DIR, SquadCorpus.NAME, "tag_list.pkl")
    ...
    def question_tag_to_ix(self, tag, is_train):
        ix = self._tag_to_ix.get(tag, 1)
        return ix

    def context_tag_to_ix(self, tag, is_train):
        # print(word)
        ix = self._tag_to_ix.get(tag, 1)
        return ix
    ...
    def _init(self):
        ...
        unk_embed = tf.get_variable(shape=(1, dim), name="unk_embed_for_tag",
                                    dtype=np.float32, trainable=self.learn_unk,
                                    initializer=tf.random_uniform_initializer(-self.tag_vec_init_scale,
                                                                              self.tag_vec_init_scale))
        ix = 2
        ...
        if self._special_tokens is not None and len(self._special_tokens) > 0:
            ...
            tok_embed = tf.get_variable(shape=(len(self._special_tokens), dim), name="token_embed_for_tag",
                                        dtype=np.float32, trainable=True,
                                        initializer=tf.random_uniform_initializer(-self.tag_vec_init_scale,
                                                                                  self.tag_vec_init_scale))
            matrix_list.append(tok_embed)
            for token in self._special_tokens:
                self._tag_to_ix[token] = ix
                ix += 1

        tag_embed = tf.get_variable(shape=(tag_dim, dim), name="tag_embed",
                                    dtype=np.float32, trainable=True,
                                    initializer=tf.random_uniform_initializer(-self.tag_vec_init_scale,
                                                                              self.tag_vec_init_scale))
        for tag in voc:
            self._tag_to_ix[tag] = ix
            ix += 1

        print("Had randomly initialized tag embeddings for %d tags" % (len(voc)))

        matrix_list.append(tag_embed)

        self._tag_emb_mat = tf.concat(matrix_list, axis=0)

    def embed(self, is_train, *tag_ix):
        if any(len(x) != 2 for x in tag_ix):
            raise ValueError()
        mat = self._tag_emb_mat
        ...
        if self.cpu:
            with tf.device("/cpu:0"):
                return [tf.nn.embedding_lookup(self._tag_emb_mat, x[0]) for x in tag_ix]
        else:
            return [tf.nn.embedding_lookup(self._tag_emb_mat, x[0]) for x in tag_ix]

    def __getstate__(self):
        state = dict(self.__dict__)
        state["_tag_emb_mat"] = None  # we will rebuild these anyway
        state["_tag_to_ix"] = None
...
class FilteredFixedWordEmbedder(FixedWordEmbedder):

    def set_proportion(self, proportion):
        self.proportion = proportion

    def set_vocab(self, corpus, loader: ResourceLoader, special_tokens: List[str]):
        if special_tokens is not None:
            self._special_tokens = sorted(special_tokens)

        proportion = self.proportion if hasattr(self, "proportion") else 0.9

        w_counts = corpus.get_word_counts() # counter returned by CorpusStats
        unknown_word_cnt = int((1-proportion) * len(w_counts))
        filtered_words, _ = zip(*w_counts.most_common()[:-unknown_word_cnt-1:-1])
        self.filter_for_unk = set(filtered_words)
    ...
    def _init(self, loader: ResourceLoader, voc: Iterable[str]):
        ...
        filtered = 0
        for word in voc:
            if word in self.filter_for_unk or word.lower() in self.filter_for_unk:
                filtered += 1
                continue
            if word in self._word_to_ix:
                continue  # in case we already added due after seeing a capitalized version of `word`
        ...
        print("Had pre-trained word embeddings for %d of %d words, filtered %d words" % (len(mat), len(voc), filtered))
